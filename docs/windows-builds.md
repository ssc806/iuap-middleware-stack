...
- `build/openresty/windows/build.sh`：OpenResty Windows 构建与打包脚本，也包含可直接编辑的完整 `configure` 参数数组
...
- 直接编辑 `build/openresty/windows/build.sh` 顶部的 `configure_args` 数组
- 该数组是 Windows OpenResty 构建时使用的完整 `./configure` 参数列表
- 可以直接新增、删除、替换 `--with-...`、`--without-...`、`--add-module=...`
- 如果新增的是第三方模块源码目录，需要自行保证该目录在 runner 上存在，或在脚本中补充下载/解压逻辑
